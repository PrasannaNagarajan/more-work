import os
os.environ["OPENAI_API_KEY"] = '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'
...
def send_click():
    st.session_state.values = youtube_link.split("v=")[1][:11]
...
if st.session_state.values != " ":
    ...
    srt = YouTubeTranscriptApi.get_transcript(st.session_state.values, languages=['en'])
    ...
    hti = Html2Image()
    hti.screenshot(url=f"https://www.youtube.com/watch?v={st.session_state.values}", save_as=youtube_img)
